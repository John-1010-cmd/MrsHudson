...
public class RouteService {
  ...
  public RouteService(WeatherProperties weatherProperties) {
  ...
  public enum TravelMode {
    WALKING("walking", "步行"), DRIVING("driving", "驾车"), TRANSIT("transit/integrated", "公交");
  ...
  private String callDirectionApi(String origin, String destination, TravelMode mode,
    ...
    try {
      ...
      UriComponentsBuilder uriBuilder = UriComponentsBuilder.fromHttpUrl(apiUrl)
          .queryParam("key", weatherProperties.getApiKey()).queryParam("origin", origin)
          .queryParam("destination", destination).queryParam("output", "JSON");
      ...
      log.error("调用路线规划API失败, origin={}, destination={}, mode={}", origin, destination, mode.name(),
          e);
   ...
   * 解析路线结果
   */
  private String parseRouteResult(JSONObject data, TravelMode mode, String originName,
      String destName) {
      ...
      for (int i = 0; i < segmentCount; i++) {
        ...
        if (walking != null) {
          ...
          if (walkDist != null && !walkDist.isEmpty()) {
            result.append(String.format("%d. 🚶 步行至%s（%s）\n", stepNum++, walkDestDesc,
                formatDistance(walkDist)));
          }
        }
  ...
  private String getCoordinate(String address) {
    try {
      ...
      URI uri = UriComponentsBuilder.fromHttpUrl(weatherProperties.getBaseUrl() + "/geocode/geo")
          .queryParam("key", weatherProperties.getApiKey()).queryParam("address", address)
          .queryParam("output", "JSON").encode().build().toUri();
      log.info("getCoordinate uri: {}", uri);
      ResponseEntity<String> response = restTemplate.getForEntity(uri, String.class);
      String responseBody = response.getBody();
      JSONObject data = JSON.parseObject(responseBody);

      if (!"1".equals(data.getString("status"))) {
        log.warn("地理编码失败: info={}, address={}, response={}", data.getString("info"), address,
            responseBody);
        return null;
      }
