...
| | | **四个状态字段互斥**：`url` 有值 / `timeout: true` / `error` 有值 / `noaudio: true`，每次仅一个有效 |
...
```
data: {"type":"cache_hit","content":"1+1 = **2**"}\n\n
...
> **注意**：缓存命中无实际 AI 调用，不发 `token_usage` 事件。
...
```
data: {"type":"clarification","content":"请问您想查询哪个城市的天气？"}\n\n
data: {"type":"done"}\n\n
```

> **注意**：意图澄清无 TTS 合成，不发 `content_done` / `audio_done` / `token_usage`。
