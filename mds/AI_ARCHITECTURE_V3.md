...
StreamChatController.streamSendMessage()
  ...
  └── 调用 StreamChatService.streamSendMessage()
        ...
        │     ⚠️ 注：IntentRouter 兼具意图识别与响应生成职责，随意图类型增加维护成本
        │        上升，V2 计划拆分为独立的识别层与处理层
        │
        ...
        │     ⚠️ 注：压缩逻辑已实现但主流程未接入，长对话上下文当前为硬截断静默丢失
        │
        ...
        │     ⚠️ 注：QualityOptimizer（控制参数）与 ModelRouter（控制模型）相互独立，
        │        存在模式与模型不匹配风险，详见 3.3 节
        │
        └─ ⑨ 执行流式 AI 调用 (executeStreamingAiCallWithTools)
              ...
              │        ⚠️ 无最大深度硬限制，依赖 AI 不再返回 TOOL_CALL 自然终止
              │        若 AI 持续返回 TOOL_CALL（模型异常或 prompt 注入），存在无限递归风险
              │        见待完善项 P1
              │
              ├── 客户端断开（doOnCancel）：
...
| 子系统 | 包路径 | 职责 | 状态 |
|--------|--------|------|------|
| **意图路由** | `optim.intent` | 三层路由：规则 → 轻量AI → 完整AI，直接处理已知意图，避免不必要的 AI 调用 | ✅ 已实现 |
| **意图向量缓存** | `optim.intent` | 意图识别结果持久化到向量数据库，三级缓存（L1内存/L2 Redis/L3向量搜索），含查询归一化和熔断降级 | 🚧 规划中 |
| **响应缓存** | `optim.cache` + `optim.cost` | 语义缓存（向量相似度匹配），ToolCacheManager，缓存失效监听 | ✅ 已实现 |
| **上下文压缩** | `optim.compress` + `optim.context` | 消息 > 15条触发压缩，早期消息摘要化，保留最近6条 | ⚠️ 已实现但主流程未接入，见 P1 |
| **自纠错** | `optim.correction` | 工具执行结果验证 → 纠错重试 → 错误模式学习 | ✅ 已实现 |
| **降级兜底** | `optim.fallback` | 工具失败 / AI 异常时的 FallbackDecision + FallbackHandler | ✅ 已实现 |
| **质量/成本** | `optim.quality` + `optim.monitor` + `optim.cost` | 质量模式三档（SPEED/BALANCED/QUALITY）、模型动态路由、Token 统计、成本监控 | ⚠️ 已实现但存在割裂问题，见 3.3 节 |
...
> ⚠️ **已知设计问题**：`IntentRouter` 当前兼具**意图识别**与**响应生成**两种职责——L1/L2 命中时直接生成完整响应内容并返回，既是路由器又是处理器。随意图类型增多，`AbstractIntentHandler` 处理器链膨胀，测试和维护成本上升。V2 计划拆分为独立的 `IntentStage`（识别）与 Handler（生成）。
...
> ⚠️ **已知设计问题**：`QualityOptimizer` 仅控制推理参数（`maxTokens` / `temperature`），`ModelRouter` 独立控制模型选择，两者**当前无联动**。典型风险场景：
> - 切换到 QUALITY 模式 + 发送短消息（≤20字）→ `ModelRouter` 仍选小模型（`moonshot-v1-8k`），高参数配置在小模型上效果有限
> - 切换到 SPEED 模式 + 发送长文本（>100字）→ `ModelRouter` 仍选大模型（`moonshot-v1-32k`），降本意图落空
>
> V2 由 `StrategyEngine` 统一模型选择与参数控制，解决此割裂问题。

### 3.4 缓存体系

系统存在三类缓存，当前各自独立管理，尚无统一 Key 规范和一致性策略：

| 缓存类型 | 实现类 | 用途 | 当前 TTL | Key 规范 |
|---------|--------|------|---------|---------|
| 语义响应缓存 | `SemanticCacheService` | AI 完整响应缓存，向量相似度匹配命中 | 待补充 | 待补充 |
| 工具结果缓存 | `ToolCacheManager` | 工具调用结果缓存，相同入参复用 | 待补充 | 待补充 |
| 意图识别缓存 | `IntentCacheStore` | 意图识别结果缓存（规划中） | L1: 5分钟 / L2: 7天 / L3: 30天 | `hash(userId + normalizedQuery)` |

> ⚠️ **已知问题**：语义响应缓存和工具结果缓存的 TTL 与 Key 规范当前未在文档中记录，各缓存无统一失效协调机制（如工具数据更新后语义缓存未主动失效）。见待完善项 P1。
...
| `done` | 流结束（所有路径均保证发送） | `{"type":"done"}` |
| ~~`audio_url`~~ | ~~已废弃，由 `audio_done` 替代~~ | — |

### 5.1 合法事件顺序

前端 / Android 实现须兼容以下各路径，**`done` 在所有路径下均保证发送**：

**① 正常对话流（无工具调用）**
```
[thinking*]  content+  content_done  audio_done  token_usage  done
```
- `thinking` 仅 reasoning 模型发送，可能出现 0 至多次，位于首个 `content` 之前
- `audio_done` 在 `content_done` 之后异步发送，前端须容忍两者之间的时间间隔

**② 工具调用流（支持多轮）**
```
[thinking*]  (tool_call  tool_result)+  content+  content_done  audio_done  token_usage  done
```
- `tool_call` 与 `tool_result` 成对出现，`tool_result` 紧随其对应的 `tool_call`
- 工具调用可多轮，每轮结束后 AI 继续生成，最终统一发送 `content_done`
- 工具纠错失败时会额外发送 `error`，但 `tool_result` 仍然发送，流程继续

**③ 缓存命中流**
```
cache_hit  content_done  audio_done  token_usage  done
```
- 命中时不发送 `content` 事件，内容直接包含在 `cache_hit` 的 `content` 字段中

**④ 意图澄清流**
```
clarification  done
```
- 不发送 `content`，不发送 `audio_done` 和 `token_usage`

**⑤ 错误流**
```
[content*]  error  done
```
- `error` 可在任意阶段发生（Provider 异常、工具不存在等）
- `error` 发生后流终止，`done` 仍保证发送
- Provider 异常时不发送 `content_done` 和 `audio_done`

**⑥ 客户端断开流**
```
[content*]  [content_done]  （服务端静默终止）
```
- 客户端断开后服务端不再发送 SSE 事件
- 若 `content_done` 已发出，TTS 在后台继续合成写 DB，但 `audio_done` 不会发送到客户端
...
4. **工具调用闭环**：AI 决定调用工具 → 执行 → 验证 → 自纠错（`correctAndRetry`）→ 兜底（`FallbackHandler` 返回友好提示）→ 结果回注 → AI 继续生成
   > ⚠️ 多轮递归当前无深度上限，依赖 AI 不再返回 `[TOOL_CALL]` 自然终止，存在失控风险，见待完善项 P1
...
| 优先级 | 事项 | 说明 | 前置依赖 |
|--------|------|------|---------|
| P0 | 意图向量缓存落地 | `IntentCacheStore` + `QueryNormalizer` + `VectorBasedRecognizer`；设计见 [intent-vector-cache-design.md](../.spec-workflow/specs/ai-agent-optimization-v2/intent-vector-cache-design.md) | Redis Vector / Chroma 环境就绪 |
...
| P1 | 工具调用递归深度限制 | 当前无硬限制，依赖 AI 自然终止，存在无限递归风险 | 无 |
| P1 | QualityOptimizer 与 ModelRouter 联动 | QUALITY 模式应约束 ModelRouter 优先选大模型，SPEED 模式应限制大模型使用 | 无 |
| P1 | 缓存策略精细化 | 补充语义缓存和工具缓存的 TTL 与 Key 规范；按工具/场景定制 TTL | 意图向量缓存落地后 |
| P1 | 意图路由模式验证 | `rule-only` / `hybrid` / `ai-only` 切换，`IntentRouterFactory` 已支持，需验证各模式行为 | 无 |
